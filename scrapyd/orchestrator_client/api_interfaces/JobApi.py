...
class JobApi:
    __endpoint_path = 'scrapyd_manager/job'
    ...
    def get(self, obj_id):
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error("Entity not found exception on GET OBJ BY ID operation")
                raise OrchestratorEntityNotFoundException("Entity not found exception on GET OBJ BY ID operation")
    ...
    def add(self, project, spider, settings):
        try:
            response = requests.request(
                'POST',
            ...
            if response.status_code == 201:
                ...
                self.authorization_api.refresh()
                return self.add(project, spider, settings)
    ...
    def update(self, obj_id, **kwargs):
        try:
            ...
            response = requests.request(
                ...
                url=f"{ORCHESTRATOR_URL}/{self.__endpoint_path}/{obj_id}/",
                headers=self.authorization_api.get_headers(),
                data=json.dumps(old_obj | kwargs, default=str)
            )
            if response.status_code == 200:
