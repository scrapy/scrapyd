...
class SpiderApi:
    __endpoint_path = 'scrapyd_manager/spider'

    def __init__(self):
        self.authorization_api = UserApi.get_instance()
        self.logger = logging.getLogger("SpiderApiInterface")
    ...
    def get_by_name(self, name, instance_id=INSTANCE_ID):
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error("Entity not found exception on GET OBJ BY NAME operation")
                return {}
    ...
    def update(self, spider_name, **kwargs):
        try:
            old_obj = self.get_by_name(spider_name)
            response = requests.request(
                'PUT',
            ...
            if response.status_code == 401:
                ...
                self.authorization_api.refresh()
                return self.update(spider_name, **kwargs)
